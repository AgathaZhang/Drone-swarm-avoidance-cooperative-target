                                                 ...
                                                 mavlink_status_t* r_mavlink_status)
{
	...
	*/
	...
	static const uint8_t mavlink_message_crcs[256][2] = MAVLINK_MESSAGE_LENGTHS_CRCS;
#define MAVLINK_MESSAGE_CRC(msgid) mavlink_message_crcs[msgid][1]
#endif
#endif
	...
	*/
	...
	static const uint8_t mavlink_message_lengths[256][2] = MAVLINK_MESSAGE_LENGTHS_CRCS;
#define MAVLINK_MESSAGE_LENGTH(msgid) mavlink_message_lengths[msgid][0]
#endif
#endif
...
//extern int uart_fd;
extern int uart0_fd;
//extern uint8_t _network_buf[MAVLINK_MAX_PACKET_LEN];
//extern unsigned _network_buf_len;
//#include "uart.h"

MAVLINK_HELPER void mavlink_send_uart_bytes(int chan, const uint8_t *buf, int len)
{
	int s_ret = -1;
    ...
    if (chan == MAVLINK_COMM_0) {
        //s_ret = write(uart0_fd, buf, len);
		//s_ret = uart_send(uart0_fd, buf, len);
    } else if (chan == MAVLINK_COMM_1) {
    	//s_ret = write(uart_fd, buf, len);
    } else if (chan == MAVLINK_COMM_2) {
    	// if (_network_buf_len + len < sizeof(_network_buf) / sizeof(_network_buf[0])) {
		// 	memcpy(&_network_buf[_network_buf_len], buf, len);
		// 	_network_buf_len += len;
		// }
    }

	//printf("mavlink_send_uart_bytes [%d]\n", s_ret);
    if (s_ret == len)
    {

    } else {
        //printf("mavlink send to uart%d err s_ret %d  len %d \r\n", chan, s_ret, len);
    }
