...
{   
    ...
    {    
        ...
        {   
            ...
            auto vector_seg = segmentVector(position, SET3D_TO_VEC3D(target), limit.constraint_speed);          // 向量分段 <vec3d> vector_seg
            auto increment = vector_seg[1] - position;                                                          // 取增量   <vec3d> increment
            Mint guide_target = QUANTIZATION_MAPPING_3D(increment);   // 输入一个vec3d的数据 输出一个实数       // 量化映射 <Mint> x y z
            auto guide_target_final = Mint2DToVec2I(MintToMin2D(guide_target));
            // step1 起点终点小数已知 √
            ...
            std::vector<Mint2D> wall;
            for (size_t i = 0; i < 60; i++)         // todo 向后找多少帧
            {   
                for (size_t j = 0; j < ALL_DRONE_NUM; j++)      // 看看是不是所有飞机都遍历到了
                {
                    vec3d dyschronism = SET3D_TO_VEC3D(matrix[frame-1+i][j]);                                    // 时间上找到障碍帧
                    if (8 > (euclideanDistance(position, dyschronism)))                                          // 帧筛选 这里设置规避的障碍半径
                    ...
                    // 目标位置不能和障碍是同一个
                    auto Box = Mint2DToVec2I(/*2小数变整Mint2D */QUANTIZATION_MAPPING_2D(/*1直接找出2D障碍小数位置*/VEC3D_TO_VEC2D(dyschronism - position)));
                    if (Box == guide_target_final) continue;        // 屏蔽此刻有飞机
            ...
            // 思考未来时刻frame z 上的碰撞
            for(auto& coordinate : path_vec3d) {
